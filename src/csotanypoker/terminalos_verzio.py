...
class jatek():
    ...
    def pakli_generalo(self):
        for i in TIPUS:
            self.pakli.extend([kartya(i) for _ in range(4)])
    ...
    def celzott_jatekos_valasztas(self):
    ...
    def kartya_valasztas(self):
        ...
        for kartya in self.aktiv_jatekos.kezbenlevo_kartyak:
            if kartya.nev == valasztott_kartya_nev:
                self.kerdeses_kartya = kartya
                self.aktiv_jatekos.kezbenlevo_kartyak.remove(kartya)  # Eltávolítjuk a kártyát
                return
        
        print("Hibás választás! Nincs ilyen kártya a kezedben.")
        self.kartya_valasztas()  # Újraválasztás
    def volt_e_nala(self):
        return self.kerdeses_kartya.volt_ennel_mar==self.celzott_jatekos
    ...
    def jatek_ciklus(self):
        ...
        while not self.van_4_lap_elotte() and self.van_lap_a_kezeben() :
            ...
            print( [f"{jatekos.nev}" for jatekos in self.jatekosok if jatekos != self.aktiv_jatekos])
    ...
    def ciklus(self):
        ...
        print(f" ez a kártya kiknél volt: {[jatekos.nev for jatekos in self.kerdeses_kartya.volt_ennel_mar]}")

        
        if len(self.kerdeses_kartya.volt_ennel_mar)==len(self.jatekosok)-1 or len(self.jatekosok)==2:
            print("IGAZ , HAMIS")
        else:
