...
from csotanypoker.client.drawing_helpers.constans import (
    DARK_GREEN, LIGHT_GREEN, LIGHT_GREEN_TRANSPARENT, LIGHTER_GREEN_TRANSPARENT,
    MIDDLE_GREEN, MIDDLE_GREEN_TRANSPARENT_50,
    MIDDLE_GREEN_TRANSPARENT_90, RED, WHITE, JATEKSZABALY,RED_TRANSPARENT
)

from csotanypoker.client.drawing_helpers.drawing_helpers import (
    _draw_rounded_rect, draw_button, draw_sound_volume, draw_text, draw_image, draw_logout_button,
    draw_rules_button, draw_rules_popup, create_volume_button_rect,
    draw_image_button, load_image
)
...
class GameDrawer:
    ...
    def draw_all(self):
        ...
        if self.screen.show_rules:
            draw_rules_popup(
                ...
                self.screen.client.height,
                JATEKSZABALY[
                    "2" if self.screen.client.selected_room.max_player_count == 2 else "3-6"
                ],
                self.screen.client.selected_room.max_player_count,
            )
    
    def _draw_center_and_mini_cards(self):
        question_card = (self.screen.local_question_card or 
                        self.screen.client.game_state.question_card)
        targeted_player = (self.screen.local_targeted_player or 
                          self.screen.client.game_state.targeted_player)
        
        if question_card and (
            targeted_player == self.screen.client.user.username or
            self.screen.client.game_state.active_player == self.screen.client.user.username
        ):
    ...
    def _draw_center_card_area(self):
        ...
        card_to_show = (self.screen.local_question_card or 
                       self.screen.client.game_state.question_card)
        if not card_to_show:
            return
        
        flip_data = self.screen.animation_manager.get_flip_data('center_card', card_to_show)
        base_width, base_height = int(65 * 2.5), int(100 * 2.5)
        
        if not flip_data['is_animating']:
            card = load_image(flip_data['current_card'], "card", size=(base_width, base_height))
            draw_image(
                self.screen.client.window, card,
        ...
        current_card = (flip_data['previous_card'] if progress < 0.5 
    ...
    def _draw_tipp_area(self):
        ...
        self.screen.cross_rect = draw_image_button(
            self.screen.client.window, "x", "button",
            self.screen.client.width // 2 - 130,
            self.screen.client.height // 2 + 60,
            (70, 70), border_radius=0, centered=True,
        ...
        self.screen.checkmark_rect = draw_image_button(
            self.screen.client.window, "pipa", "button",
            self.screen.client.width // 2 + 130,
            self.screen.client.height // 2 + 60,
            (80, 80), border_radius=0, centered=True,
            is_hovered="checkmark" in self.screen.hovered_elements,
            is_active="checkmark" in self.screen.pressed_elements,
        )
        
        if len(self.screen.client.game_state.visited_already) < (len(self.screen.client.users) - 1):
            draw_button(
    ...
    def _draw_card_in_hand(self):
        ...
        if (not hasattr(self.screen.client, 'visible_player') 
    ...
    def _draw_mini_card(self):
        ...
        if (not (self.screen.local_question_card or self.screen.client.game_state.question_card)
            or not (self.screen.local_targeted_player or self.screen.client.game_state.targeted_player)
            or self.screen.client.game_state.active_player == self.screen.client.user.username):
            return
        
        if not hasattr(self.screen, "opponent_player_rects") or not self.screen.opponent_player_rects:
            return
        
        target_player = (self.screen.local_targeted_player or 
                        self.screen.client.game_state.targeted_player)
        ...
        if not flip_data['is_animating']:
            card_image = load_image(flip_data['current_card'], "card", 
                                   size=(base_width, base_height))
            draw_image(
                self.screen.client.window, card_image,
        ...
        for rect, player_name in self.screen.opponent_player_rects:
            if player_name == self.screen.client.game_state.active_player:
                ...
                if active_statement and active_statement[0] is not None:
                    statement_text = f"Ez egy {self._translate_animal(str(active_statement[0]))}"
                    self._draw_statement_bubble(
                        rect.centerx, rect.bottom + 15,
                        statement_text, WHITE, MIDDLE_GREEN_TRANSPARENT_90
                    )
                break
        ...
        for rect, player_name in self.screen.opponent_player_rects:
            if player_name == self.screen.client.game_state.targeted_player:
                ...
                if target_answer[0] is not None:
                    ...
                    statement_text = ("Igen, hiszek neked." if target_answer[0] 
                                     else "Nem hiszek neked.")
                    self._draw_statement_bubble(
                        rect.centerx, statement_y,
                        statement_text, DARK_GREEN, LIGHTER_GREEN_TRANSPARENT
                    )
                break
    ...
    def _draw_info_messages(self):
        if (not hasattr(self.screen.client, 'game_state')
    ...
    def _draw_animal_table(self):
        ...
        table_x, table_y = 25, self.screen.client.height // 2 - 135
        table_width, table_height = 200, 320
        ...
        button_spacing_x, button_spacing_y = 90, 65
    ...
    def _draw_opponent_players(self):
        if (not hasattr(self.screen.client, 'opponent_players')
        ...
        for i, player in enumerate(self.screen.client.opponent_players):
            ...
            if (len(self.screen.client.opponent_players) == 1
                and self.screen.client.user.username == self.screen.client.game_state.active_player):
    ...
    def _draw_bottom_buttons(self):
        ...
        volume_rect = create_volume_button_rect(
            self.screen.client.width - 65,
            self.screen.client.height - 165
        )
        draw_sound_volume(
            self.screen.client.window,
            volume_rect.centerx,
