...
class GameStateManager:
    ...
    def reset_local_selections(self):
    ...
    def update_on_player_change(self):
        ...
        if not is_my_turn:
            ...
            self.screen.adott = False
            if (hasattr(self.screen.client, 'opponent_players')
                and self.screen.client.opponent_players
                and len(self.screen.client.opponent_players) == 1):
                self.screen.local_targeted_player = self.screen.client.opponent_players[0].username
    
    def check_leave_button_visibility(self):
        """Ellenőrzi, kell-e megjeleníteni az elhagyás gombot"""
    ...
    def set_contextual_message(self):
        """Kontextusfüggő üzenet beállítása"""
        if (not hasattr(self.screen.client, "message")
