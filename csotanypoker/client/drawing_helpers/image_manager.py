...
def preload_all_images():
    for animal in Animal:
        _get_cached_image(str(animal), "logo")

        _get_cached_scaled_image(str(animal), "logo", (50, 50))
        _get_cached_scaled_image(str(animal), "logo", (58, 58))
        _get_cached_scaled_image(str(animal), "logo", (64, 64))  # hover
        _get_cached_scaled_image(str(animal), "logo", (70, 70))  # active

    for animal in Animal:
        _get_cached_image(str(animal), "card")

        card_size = (int(650 * 0.17), int(1000 * 0.17))
        _get_cached_scaled_image(str(animal), "card", card_size)
    ...
    for element in ui_elements:
        _get_cached_image(str(element), "button")

        _get_cached_scaled_image(str(element), "button", (35, 35))
        _get_cached_scaled_image(str(element), "button", (40, 40))
        _get_cached_scaled_image(str(element), "button", (50, 50))
    background_types = [
        "light_background",
        "background",
    ]
    for bg_type in background_types:
        _get_cached_image(str(bg_type), "background")
...
def _get_cached_image(name: str, image_type: str) -> pygame.Surface:
    ...
    if cache_key not in _image_cache:
        ...
        path = resource_path(
            os.path.join("csotanypoker", "client", "images", folder, filename)
        )
